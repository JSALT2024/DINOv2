...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.img_dir == '56':
                image_pil = image_pil.resize((56, 56))

            #logger.info(f"success in reading image: {img_path}")
        except Exception as e:
            img_path = "/share/data/2pals/shester/crops/face/yasl/imgs64/ZRu5_9uLGpM.006298-006511_hand2_frame_21.jpg"
            
            #img_path = self.img_labels.iloc[0, 0]
            with open(img_path, mode="rb") as f:
                image_pil = f.read()
            image_pil = ImageDataDecoder(image_pil).decode()
            print("used second image")
            
            #logger.info(f"success in reading second image: {img_path}")
            
            # logger = logging.getLogger("dinov2")
            # img_path = self.img_labels.iloc[idx, 0]
            # logger.info(f"error in reading image: {img_path}")
