...
def do_train(cfg, model, config_file_path, resume=False): # change resume to true?
    model.train()
    ...
    if cfg.head.head_path:
        start_iter = int(cfg.head.head_path.rsplit('_',1)[-1]) + 1 #999 shester
    ...
    for data in metric_logger.log_every(
        ...
        if cfg.evaluation.eval_period_iterations > 0 and (iteration + 1) % cfg.evaluation.eval_period_iterations == 0:
            ...
            # replace the path of head.head_path inplace in config.yaml file with the path of this iteration           
            # Read the existing YAML content
            with open(config_file_path, 'r') as file:
                config = yaml.safe_load(file)
            
            key_path = 'head.head_path'
            keys = key_path.split('.')
            sub_config = config
            for key in keys[:-1]:  # Navigate to the last key
                sub_config = sub_config.setdefault(key, {})  # Create intermediate dicts if necessary
            sub_config[keys[-1]] = cfg.head.head_path.rsplit('_',1)[0] + '_' + str(iteration)  # Set the new value
...
def main(args):
    ...
    do_train(cfg, model, args.config_file, resume=not args.no_resume)
